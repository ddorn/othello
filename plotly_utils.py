...
def line(y: Union[t.Tensor, List[t.Tensor]], renderer=None, **kwargs):
    ...
    if "use_secondary_yaxis" in kwargs_pre and kwargs_pre["use_secondary_yaxis"]:
        ...
        y = (list(map(utils.to_numpy, y)) if isinstance(y, list)
             and not (isinstance(y[0], int) or isinstance(y[0], float)) else utils.to_numpy(y))
...
def plot_comp_scores(model,
                     comp_scores,
                     title: str = "",
                     baseline: Optional[t.Tensor] = None) -> go.Figure:
    return px.imshow(
...
def plot_logit_attribution(model: HookedTransformer,
                           logit_attr: t.Tensor,
                           tokens: t.Tensor,
                           title: str = ""):
...
def plot_contribution_vs_open_proportion(
    ...
    for name, mask in failure_types_dict.items():
        failure_types = np.where(utils.to_numpy(mask), name, failure_types)
    fig = (px.scatter(
        x=utils.to_numpy(data.open_proportion),
        y=utils.to_numpy(unbalanced_component),
        color=failure_types,
        color_discrete_map=color_discrete_map,
        title=title,
        template="simple_white",
...
def mlp_attribution_scatter(
    ...
    failure_types = np.full(out_by_component_in_pre_20_unbalanced_dir.shape[-1],
                            "",
                            dtype=np.dtype("U32"))
    for name, mask in failure_types_dict.items():
        failure_types = np.where(utils.to_numpy(mask), name, failure_types)
...
def plot_neurons(
    ...
    for name, mask in failure_types_dict.items():
        failure_types = np.where(utils.to_numpy(mask[utils.to_numpy(data.starts_open)]), name,
                                 failure_types)
    ...
    neuron_numbers = einops.repeat(t.arange(model.cfg.d_model),
                                   "n -> (s n)",
                                   s=data.starts_open.sum())
    failure_types = einops.repeat(failure_types, "s -> (s n)", n=model.cfg.d_model)
    data_open_proportion = einops.repeat(data.open_proportion[data.starts_open],
                                         "s -> (s n)",
                                         n=model.cfg.d_model)
    df = pd.DataFrame({
...
def hists_per_comp(
    ...
    for ((row, col), title), in_dir in zip(titles.items(), out_by_component_in_unbalanced_dir):
        fig.add_trace(
            go.Histogram(
                x=utils.to_numpy(in_dir[data.isbal]),
                name="Balanced",
                marker_color="blue",
                opacity=0.5,
                legendgroup="1",
...
def plot_loss_difference(log_probs, rep_str, seq_len):
    fig = px.line(
        ...
        title=
        f"Per token log-prob on correct token, for sequence of length {seq_len}*2 (repeated twice)",
        labels={
